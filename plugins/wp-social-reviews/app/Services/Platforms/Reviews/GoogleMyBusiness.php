...
class GoogleMyBusiness extends BaseReview
{
    ...
    public function getApiSettings()
    {
        ...
        if (!$settings) {
            $access_token = Arr::get($apiSettings, 'access_token', '');
    ...
    public function getNextLocationsResponse($accountName, $headers, $data)
    {
        ...
        $total = apply_filters('wpsocialreviews/gmb_locations_limit', 100);
        ...
        while(!empty($this->locationNextPageToken) && $x < $pages) {
            $x++;
            $fetchUrl = "https://mybusinessbusinessinformation.googleapis.com/v1/".$accountName."/locations?readMask=name,latlng,metadata,profile,serviceItems,title,openInfo&pageToken=".$this->locationNextPageToken;
