...
class FacebookFeed extends BaseFeed
{
    ...
    public function formatPageData($page = [], $pageId = '' , $connectionType = '')
    {
        ...
        $data = [
	        ...
	        'is_event_enabled' => $connectionType === 'event_feed' ? true : false,
            'error_message'  => '',
            'status'         => 'success',
        ];
        return $data;
            ...
            if(Arr::get($pages_data, 'response.code') !== 200) {
                ...
                if(!empty($connectedSources)){
                    ...
                    foreach ($connectedSources as $key => $source){
                        ...
                        $errorMessage = Arr::get($pages_response_data, 'error.message', '');
                        if($source_account_id === $account_id && !empty($errorMessage)) {
                            $connectedSources[$key]['status'] = 'error';
                            $connectedSources[$key]['error_message'] = $errorMessage;
                        }
                    }
