...
/* have been moved to new algorithm:
-- fixed price fully
-- minmax rule selection by user, but cart parsing algorithm still poor
-- b2bking_dynamic_rule_required_multiple selection by user, but cart algorithm still poor -other required functions not moved


A priority is to bring rule selection by user to all rules
*/
...
class B2bking_Dynamic_Rules {
		...
		public static function b2bking_dynamic_rule_hidden_price($price, $product){
			...
			if (empty($hidden_price_rules) && empty($hidden_price_parent_rules)){
				...
				return apply_filters('b2bking_hidden_price_rule_text',get_option('b2bking_hidden_price_dynamic_rule_text_setting', esc_html__('Price is unavailable','b2bking')), $current_product_id);
			}
		...
		public static function b2bking_dynamic_rule_unpurchasable_disable_purchasable($purchasable, $product){
			...
			// 1) Get all rules and check if any rules apply to the product
			$rules_that_apply_to_product = get_transient('b2bking_unpurchasable_rules_apply_'.$current_product_id);
			if (!$rules_that_apply_to_product){

				$rules_that_apply = array();
				$unpurchasable_rules_option = get_option('b2bking_have_unpurchasable_rules_list_ids', '');
				if (!empty($unpurchasable_rules_option)){
					$unpurchasable_rules_v2_ids = explode(',',$unpurchasable_rules_option);
				} else {
					$unpurchasable_rules_v2_ids = array();
				}

				foreach ($unpurchasable_rules_v2_ids as $rule_id){
					$applies = get_post_meta($rule_id,'b2bking_rule_applies', true);
					if ($applies === 'cart_total' || $applies === 'product_'.$current_product_id){
						array_push($rules_that_apply, $rule_id);
					} else if ($applies === 'multiple_options'){
				...
				set_transient('b2bking_unpurchasable_rules_apply_'.$current_product_id,$rules_that_apply);
				$rules_that_apply_to_product = $rules_that_apply;
			}
			// 2) If no rules apply for product, set transient for current user to empty array
			if (empty($rules_that_apply_to_product)){
				set_transient('b2bking_unpurchasable_'.$current_product_id.'_'.get_current_user_id(), array());
			} else {
				// if transient does not already exist
				if (!get_transient('b2bking_unpurchasable_'.$current_product_id.'_'.get_current_user_id())){
					...
					// either an empty array or an array with rules
					set_transient('b2bking_unpurchasable_'.$current_product_id.'_'.get_current_user_id(), $rules_that_apply_to_user);
				}
			}
			...
			if (!get_transient('b2bking_unpurchasable_'.$current_product_id.'_'.get_current_user_id())){
				...
				if ($post_parent_id !== 0){
					...
					// based on code above
					// 1) Get all rules and check if any rules apply to the product
					$rules_that_apply_to_product = get_transient('b2bking_unpurchasable_parent_rules_apply_'.$current_product_id);
					if (!$rules_that_apply_to_product){
						...
						set_transient('b2bking_unpurchasable_parent_rules_apply_'.$current_product_id,$rules_that_apply);
						$rules_that_apply_to_product = $rules_that_apply;
					}
					// 2) If no rules apply for product, set transient for current user to empty array
					if (empty($rules_that_apply_to_product)){
						set_transient('b2bking_unpurchasable_parent_'.$current_product_id.'_'.get_current_user_id(), array());
					} else {
						// if transient does not already exist
						if (!get_transient('b2bking_unpurchasable_parent_'.$current_product_id.'_'.get_current_user_id())){
							...
							$current_product_id = $product->get_id();
							// either an empty array or an array with rules
							set_transient('b2bking_unpurchasable_parent_'.$current_product_id.'_'.get_current_user_id(), $rules_that_apply_to_user);
						}
					}
			...
			$unpurchasable_rules = get_transient('b2bking_unpurchasable_'.$current_product_id.'_'.get_current_user_id());
			$unpurchasable_parent_rules = get_transient('b2bking_unpurchasable_parent_'.$current_product_id.'_'.get_current_user_id());

			// if there are no hidden price rules, than price is not hidden, and viceversa
			if (empty($unpurchasable_rules) && empty($unpurchasable_parent_rules)){
				return $purchasable;
			} else {
				return false;
			}
		...
		public static function b2bking_dynamic_rule_cart_discount( WC_Cart $cart ){
						...
						if ($product_is_excluded === 'no'){
							...
							if ($passconditions === 'yes'){
								...
								if ($type === 'discount_amount'){
									$howmuch = round(floatval ($howmuch) * $cart_item['quantity'], $decimals);
								...
								if ($howmuch > 0){
									...
									if($discount_name !== NULL && $discount_name !== ''){
										$cart->add_fee( get_the_title($current_product_id).' '.apply_filters('b2bking_discount_name_excluding_products',$discount_name), - $howmuch);
									} else {
			...
			*/
			...
			if($current_total_cart_discount > 0){
				...
				$decimals = apply_filters('b2bking_rounding_precision', get_option('woocommerce_price_num_decimals', 2));
				$cart->add_fee( $discount_display_name, -round($current_total_cart_discount, $decimals));
			}
			...
			foreach ($category_discount_rules as $category_discount_rule){
				...
				if(is_object($cart)) {
					foreach($cart->get_cart() as $cart_item){
						...
						if(b2bking()->b2bking_has_category($category_id, 'product_cat', $cart_item['product_id'])){
		...
		public static function b2bking_dynamic_rule_bogo_discount( WC_Cart $cart ){
			...
			if(is_object($cart)) {
				...
				foreach($cart->get_cart() as $cart_item){
					...
					// 1) Get all rules and check if any rules apply to the product
					$rules_that_apply_to_product = get_transient('b2bking_bogo_discount_rules_apply_'.$current_product_id);
					if (!$rules_that_apply_to_product){

						$rules_that_apply = array();
						$bogo_discount_rules_option = get_option('b2bking_have_bogo_discount_rules_list_ids', '');
						if (!empty($bogo_discount_rules_option)){
							$bogo_discount_rules_v2_ids = explode(',',$bogo_discount_rules_option);
						} else {
							$bogo_discount_rules_v2_ids = array();
						}

						foreach ($bogo_discount_rules_v2_ids as $rule_id){
							$applies = get_post_meta($rule_id,'b2bking_rule_applies', true);
							if ($applies === 'cart_total' || $applies === 'product_'.$current_product_id){
								array_push($rules_that_apply, $rule_id);
							} else if ($applies === 'multiple_options'){
						...
						set_transient('b2bking_bogo_discount_rules_apply_'.$current_product_id,$rules_that_apply);
						$rules_that_apply_to_product = $rules_that_apply;
					}
					// 2) If no rules apply for product, set transient for current user to empty array
					if (empty($rules_that_apply_to_product)){
						set_transient('b2bking_bogo_discount_'.$current_product_id.'_'.get_current_user_id(), array());
					} else {
						// if transient does not already exist
						if (!get_transient('b2bking_bogo_discount_'.$current_product_id.'_'.get_current_user_id())){
							...
							// either an empty array or an array with rules
							set_transient('b2bking_bogo_discount_'.$current_product_id.'_'.get_current_user_id(), $rules_that_apply_to_user);
						}
					}
					...
					if (!get_transient('b2bking_bogo_discount_'.$current_product_id.'_'.get_current_user_id())){
						if ($post_parent_id !== 0){
							...
							// based on code above
							// 1) Get all rules and check if any rules apply to the product
							$rules_that_apply_to_product = get_transient('b2bking_bogo_discount_parent_rules_apply_'.$current_product_id);
							if (!$rules_that_apply_to_product){
								...
								set_transient('b2bking_bogo_discount_parent_rules_apply_'.$current_product_id,$rules_that_apply);
								$rules_that_apply_to_product = $rules_that_apply;
							}
							// 2) If no rules apply for product, set transient for current user to empty array
							if (empty($rules_that_apply_to_product)){
								set_transient('b2bking_bogo_discount_parent_'.$current_product_id.'_'.get_current_user_id(), array());
							} else {
								// if transient does not already exist
								if (!get_transient('b2bking_bogo_discount_parent_'.$current_product_id.'_'.get_current_user_id())){
									...
									// either an empty array or an array with rules
									set_transient('b2bking_bogo_discount_parent_'.$current_product_id.'_'.get_current_user_id(), $rules_that_apply_to_user);
								}
							}
					...
					$bogo_discount_rules = get_transient('b2bking_bogo_discount_'.$current_product_id.'_'.get_current_user_id());
					$bogo_discount_parent_rules = get_transient('b2bking_bogo_discount_parent_'.$current_product_id.'_'.get_current_user_id());

					if (empty($bogo_discount_rules)){
						$discount_rules = $bogo_discount_parent_rules;
						...
						$discount_rules = $bogo_discount_rules;
						$current_product_categories = b2bking()->get_all_product_categories( $current_product_id, 'product_cat' );
						if (empty($current_product_categories)){
							// if no categories, this may be a variation, check parent categories
							$possible_parent_id = wp_get_post_parent_id($current_product_id);
						...
						if (!empty($conditions)){
							...
							if ($applies[0] === 'excluding'){
								...
								foreach($current_product_belongsto_array as $element){
									if(in_array($element, $rule_multiple_options_array)){
										...
										if ($element_array[0] === 'product'){
											...
											if(is_object($cart)) {
												foreach($cart->get_cart() as $cart_item2){
													if(b2bking()->b2bking_has_category($element_array[1], 'product_cat', $cart_item2['product_id'])){
								...
								if ($applies[0] === 'category'){
									...
									if(is_object($cart)) {
										foreach($cart->get_cart() as $cart_item2){
										if(b2bking()->b2bking_has_category($applies[1], 'product_cat', $cart_item2['product_id'])){
		...
		public static function b2bking_dynamic_rule_discount_display_dynamic_sale_badge($text, $post, $product){
			...
			if($have_discounted_price !== NULL && $smallest_discount_name !== '' && $smallest_discount_name !== NULL){
				$returned = str_replace( __( 'Sale!', 'woocommerce' ), $smallest_discount_name, $text );
			} else {
				$returned = $text;
			}
				...
				if ($applies[0] === 'multiple'){
					...
					foreach ($rule_multiple_options_array as $rule_element){
						...
						if ($rule_element_array[0] === 'category'){
							...
							if(is_object($cart)) {
								foreach($cart->get_cart() as $cart_item){
								if(b2bking()->b2bking_has_category($rule_element_array[1], 'product_cat', $cart_item['product_id'])){
									$item_qty = $cart_item["quantity"];// Quantity
									$item_line_total = $cart_item["line_total"]; // Item total price (price x quantity)
									$category_products_number += $item_qty; // ctotal number of items in cart
									$category_products_value += $item_line_total; // calculated total items amount
							...
							if (isset($tax)){
								...
								if ($tax !== null && intval($tax) !== 0){
									$cart->add_fee( esc_html($taxname.' '.$category_name), round($tax, $decimals), apply_filters('b2bking_taxable_fee', false, $taxname));
								}
							}
						...
						// if is product
							...
							if (isset($tax)){
								if ($tax !== null && intval($tax) !== 0){
									...
									$cart->add_fee( esc_html($taxname.' '.$product_name), round($tax, $decimals), apply_filters('b2bking_taxable_fee', false, $taxname));
								}
							}
					...
					if (isset($tax)){
						if ($tax !== null && floatval($tax)>0){
							$decimals = apply_filters('b2bking_rounding_precision', get_option('woocommerce_price_num_decimals', 2));
							$cart->add_fee( esc_html($taxname), round($tax, $decimals), apply_filters('b2bking_taxable_fee', false, $taxname));
						}
					}
				}
		...
		public static function b2bking_dynamic_rule_fixed_price( $price, $product ) {
			...
			$user_applicable_rules = get_transient('b2bking_fixed_price_user_applicable_rules_'.$user_id);
			if (!$user_applicable_rules){
				$rules_ids_elements = get_option('b2bking_have_fixed_price_rules_list_ids_elements', array());
				...
				set_transient('b2bking_fixed_price_user_applicable_rules_'.$user_id,$user_applicable_rules);
			}
			...
			*/
			...
			if (!get_transient('b2bking_fixed_price_'.$current_product_id.'_'.get_current_user_id())){
				...
				set_transient('b2bking_fixed_price_'.$current_product_id.'_'.get_current_user_id(), $final_rules);
			}
			...
			if (!get_transient('b2bking_fixed_price_'.$current_product_id.'_'.get_current_user_id())){
				...
				if ($post_parent_id !== 0){
					...
					if (!get_transient('b2bking_fixed_price_parent_'.$current_product_id.'_'.get_current_user_id())){
						...
						set_transient('b2bking_fixed_price_parent_'.$current_product_id.'_'.get_current_user_id(), $final_rules);
					}
	...
	public static function b2bking_dynamic_minmax_order_amount_quantity_variation( $args, $instance, $product ) {
		...
		foreach($dynamic_minmax_rules as $dynamic_minmax_rule){
			...
			if ($applies === 'cart_total'){
				...
				if ($applies[0] === 'category'){
					// rule is category rule
					if(b2bking()->b2bking_has_category($applies[1], 'product_cat', $product_id)){
						if ($quantity_value === 'quantity'){
							if ($minimum_maximum === 'maximum_order'){
								if ($largest_maximum === 'none'){
									$largest_maximum = $howmuch;
				...
				// multiple select rule
					...
					foreach($rule_multiple_options_array as $rule_element){
						...
						if ($rule_element_array[0] === 'category'){
							...
							if(b2bking()->b2bking_has_category($rule_element_array[1], 'product_cat', $product_id)){
								if ($quantity_value === 'quantity'){
									if ($minimum_maximum === 'maximum_order'){
										if ($largest_maximum === 'none'){
											$largest_maximum = $howmuch;
		...
		if($have_required_multiple !== NULL){
			...
			if ($current_parentproduct_id !== 0){
				...
				if (!$rules_that_apply_to_product){
					...
					foreach ($required_multiple_rules_v2_ids as $rule_id){
						$applies = get_post_meta($rule_id,'b2bking_rule_applies', true);
						if ($applies === 'cart_total' || $applies === 'product_'.$current_parentproduct_id){
							array_push($rules_that_apply, $rule_id);
						} else if ($applies === 'multiple_options'){
							$multiple_options = get_post_meta($rule_id,'b2bking_rule_applies_multiple_options', true);
							$multiple_options_array = explode(',', $multiple_options);
							if (in_array('product_'.$current_parentproduct_id, $multiple_options_array)){
								...
								// try categories
								$current_product_categories = b2bking()->get_all_product_categories( $current_parentproduct_id, 'product_cat' );
								if (empty($current_product_categories)){
									// if no categories, this may be a variation, check parent categories
	...
	public static function b2bking_dynamic_rule_required_multiple_quantity_number( $quantity, $product_id ) {
			...
			if (!get_transient('b2bking_required_multiple_'.$current_product_id.'_'.get_current_user_id())){
				...
				if ($post_parent_id !== 0){
					...
					// based on code above
					// 1) Get all rules and check if any rules apply to the product
					$rules_that_apply_to_product = get_transient('b2bking_required_multiple_parent_rules_apply_'.$current_product_id);
					if (!$rules_that_apply_to_product){
						...
						set_transient('b2bking_required_multiple_parent_rules_apply_'.$current_product_id,$rules_that_apply);
						$rules_that_apply_to_product = $rules_that_apply;
					}
					// 2) If no rules apply for product, set transient for current user to empty array
					if (empty($rules_that_apply_to_product)){
						set_transient('b2bking_required_multiple_parent_'.$current_product_id.'_'.get_current_user_id(), array());
					} else {
						// if transient does not already exist
						if (!get_transient('b2bking_required_multiple_parent_'.$current_product_id.'_'.get_current_user_id())){
							...
							$current_product_id = $product_id;
							// either an empty array or an array with rules
							set_transient('b2bking_required_multiple_parent_'.$current_product_id.'_'.get_current_user_id(), $rules_that_apply_to_user);
						}
					}
			...
			$required_multiple_rules = get_transient('b2bking_required_multiple_'.$current_product_id.'_'.get_current_user_id());
			$required_multiple_parent_rules = get_transient('b2bking_required_multiple_parent_'.$current_product_id.'_'.get_current_user_id());

			if (empty($required_multiple_rules)){
				$required_multiple_rules = $required_multiple_parent_rules;
				$current_product_categories = b2bking()->get_all_product_categories( $post_parent_id, 'product_cat' );
				if (empty($current_product_categories)){
					// if no categories, this may be a variation, check parent categories
					$possible_parent_id = wp_get_post_parent_id($current_product_id);
				...
				$required_multiple_rules = $required_multiple_rules;
				$current_product_categories = b2bking()->get_all_product_categories( $current_product_id, 'product_cat' );
				if (empty($current_product_categories)){
					// if no categories, this may be a variation, check parent categories
					$possible_parent_id = wp_get_post_parent_id($current_product_id);
					...
					if ($rule_element_array[0] === 'category'){
						...
						if ($passconditions === 'yes'){
							...
							if ( (b2bking()->custom_modulo($category_products_number, $howmuch) ) > 0) {
								...
								wc_add_notice( sprintf( esc_html__('Products in the %s category must be purchased in multiples of %s products: '.$names_list, 'b2bking'), $category_name, $howmuch ), 'error' );
							}
						}
