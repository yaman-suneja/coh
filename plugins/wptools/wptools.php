...
if(is_admin()) {
	...
	add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'stopbadbots_add_action_links22');
	function stopbadbots_add_action_links22($links)
	{
		$mylinks = array(
...
if (is_admin()) {
	...
	function wptools_alert_errors()
	{
		...
		$args = array(
			'id' => 'wptools-alert',
			'title' => '<div class="wptools-alert-logo"></div><span id="wptools_alert_errors" class="text">'. esc_attr__("Errors","wptools").'</td>',
			'href' => $site,
			'meta' => array(
				'class' => 'wptools-alert',
				'title' => ''
...
/*
function myplugin_wptools_is_enabled(){
