document.addEventListener('readystatechange', function () {
    setInterval(function () {
        ...
        let checkoutTotal = checkoutTotalParent.querySelector("." + sezzle_installment_widget.order_total_container_class_name), // WooCommerce
            installmentBox = document.querySelector('#sezzle-installment-widget-box');
        if (!checkoutTotal || !installmentBox) {
            return;
        }
        let merchantLocale = sezzle_installment_widget.merchant_locale,
            currency = sezzle_installment_widget.currency,
            language = document.querySelector('html').lang.substring(0, 2).toLowerCase() || navigator.language.substring(0, 2) || 'en';
        if (!installmentBox.querySelector('.sezzle-payment-schedule-container')) {
            ...
            merchantLocale = merchantLocale || document.querySelector('html').lang.split('-')[1] || 'US';
		...
		`;
            ...
            for (let i = 0; i < 3; i++) {
                var installmentDate = new Date(todaysDate.setDate(todaysDate.getDate() + interval)).toLocaleDateString(language, {
                    month: 'short',
                    day: 'numeric'
                });
