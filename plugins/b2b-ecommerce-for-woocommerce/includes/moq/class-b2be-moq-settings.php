...
class B2BE_MOQ_Settings {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_save_moq_rules', __CLASS__ . '::save_moq_rules' );
		add_action( 'wp_ajax_nopriv_save_moq_rules', __CLASS__ . '::save_moq_rules' );
	...
	public static function get_settings() {
		...
		include CWRFQ_PLUGIN_DIR . '/includes/admin/moq/views/moq-fields.php';
	 ...
	 */
	public static function save_moq_rules() {

		if ( ! empty( $_POST['_wpnonce'] ) ) {
			wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ) );
		}

		$b2be_moq_rules = filter_input( INPUT_POST, 'moqRules', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY );
		$is_enable      = isset( $_POST['isEnable'] ) ? sanitize_text_field( wp_unslash( $_POST['isEnable'] ) ) : '';

		if ( 'false' == $is_enable ) {
			update_option( 'b2be_enable_moq', $is_enable );
			wp_die( 'true' );
		}

		if ( ! empty( $b2be_moq_rules ) ) {

			$is_validated = self::b2be_validate_moq_rules( $b2be_moq_rules );
			if ( $is_validated ) {

				delete_option( 'b2be_moq_rule' );
				update_option( 'b2be_moq_rule', $b2be_moq_rules );
	 ...
	 * Validating Rules.
	 *
	 * @param array $b2be_moq_rules Moq rules array.
	 */
	public static function b2be_validate_moq_rules( $b2be_moq_rules = array() ) {
		...
		$common_roles      = array_column( $b2be_moq_rules, 'roles', 'ruleId' );
		$common_categories = array_column( $b2be_moq_rules, 'categories', 'ruleId' );
		$common_customer   = array_column( $b2be_moq_rules, 'customer', 'ruleId' );
		$common_product    = array_column( $b2be_moq_rules, 'products', 'ruleId' );
		$common_quantity   = array_column( $b2be_moq_rules, 'innerRule', 'ruleId' );
		...
		$b2be_moq_rules_length = count( $b2be_moq_rules );
		for ( $i = 0; $i <= $b2be_moq_rules_length; $i++ ) {
			for ( $j = $i + 1; $j <= $b2be_moq_rules_length; $j++ ) {
				if ( ! empty( array_intersect( $b2be_moq_rules[ $i ], $b2be_moq_rules[ $j ] ) ) ) {
					$common_problems['is_role_based']     = ( $b2be_moq_rules[ $i ]['is_role_based'] == $b2be_moq_rules[ $j ]['is_role_based'] );
					$common_problems['is_product_based']  = ( $b2be_moq_rules[ $i ]['is_product_based'] == $b2be_moq_rules[ $j ]['is_product_based'] );
					$common_problems['is_category_based'] = ( $b2be_moq_rules[ $i ]['is_category_based'] == $b2be_moq_rules[ $j ]['is_category_based'] );
					$common_problems['is_customer_based'] = ( $b2be_moq_rules[ $i ]['is_customer_based'] == $b2be_moq_rules[ $j ]['is_customer_based'] );
		...
		for ( $i = 0; $i < $error_length; $i++ ) {
			foreach ( $b2be_moq_rules as $key => $value ) {
				$index = array_search( $errors[ $i ], $value );
				if ( 'ruleId' == $index ) {
					$priority_found[ $b2be_moq_rules[ $key ]['ruleId'] ] = $b2be_moq_rules[ $key ]['priority'];
				}
			}
		}
