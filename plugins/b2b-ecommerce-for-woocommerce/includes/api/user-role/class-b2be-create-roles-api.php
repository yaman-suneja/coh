<?php
/**
 * API File For Creating roles.
 *
 * @package class-b2be-create-roles-api.php
 */
...
class B2BE_Create_Roles_API {
	...
	/**
	 * Create Role according to params provided.
	 *
	...
	public function create_role( WP_REST_Request $request ) {
		...
		if ( role_exists( $role_name ) ) {
			$response['status'] = 409;
			$errors             = __( 'Role Name alredy exist.' );
			return new WP_Error( __( 'Something went wrong.' ), $errors, $response );
		}
		...
		$b2be_role_id = wp_insert_post( $args );
		add_role( $role_name, $parameters['name'], array( 'read' => true ) );
