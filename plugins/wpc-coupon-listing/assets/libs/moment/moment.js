...
;(function(global, factory) {
  ...
  addParseToken('YYYY', function(input, array) {
    array[YEAR] = input.length === 2 ? hooks.parseTwoDigitYear(input) : toInt(
        input);
  });
  ...
  function daysInMonth(year, month) {
    ...
    return modMonth === 1 ? (isLeapYear(year) ? 29 : 28) : (31 - modMonth % 7 %
        2);
  }
  ...
  function checkWeekday(weekdayStr, parsedInput, config) {
    if (weekdayStr) {
      ...
      var weekdayProvided = defaultLocaleWeekdaysShort.indexOf(weekdayStr),
          weekdayActual = new Date(parsedInput[0], parsedInput[1],
  ...
  function configFromRFC2822(config) {
    ...
    if (match) {
      var parsedArray = extractFromRFC2822Strings(match[4], match[3], match[2],
  ...
  function isDurationValid(m) {
    for (var key in m) {
      if (!(indexOf.call(ordering, key) !== -1 &&
  ...
  function cloneWithOffset(input, model) {
    ...
    if (model._isUTC) {
      res = model.clone();
      diff = (isMoment(input) || isDate(input) ? input.valueOf() : createLocal(
          input).valueOf()) - res.valueOf();
  ...
  function isBetween(from, to, units, inclusivity) {
    ...
    return (inclusivity[0] === '('
        ...
        (inclusivity[1] === ')' ? this.isBefore(localTo, units) : !this.isAfter(
            localTo, units));
  }
  ...
  function toISOString(keepOffset) {
    ...
    if (isFunction(Date.prototype.toISOString)) {
      ...
      if (utc) {
        ...
        return new Date(
            this.valueOf() + this.utcOffset() * 60 * 1000).toISOString().
  ...
  function endOf(units) {
    ...
    switch (units) {
      case 'year':
        ...
        break;
      case 'quarter':
        time = startOfDate(this.year(), this.month() - this.month() % 3 + 3,
            1) - 1;
        break;
      case 'month':
      ...
      case 'hour':
        time = this._d.valueOf();
        time += MS_PER_HOUR -
            mod$1(time + (this._isUTC ? 0 : this.utcOffset() * MS_PER_MINUTE),
                MS_PER_HOUR) - 1;
        break;
      case 'minute':
        time = this._d.valueOf();
  ...
  proto.dates = deprecate('dates accessor is deprecated. Use date instead.',
      getSetDayOfMonth);
  proto.months = deprecate('months accessor is deprecated. Use month instead',
      getSetMonth);
