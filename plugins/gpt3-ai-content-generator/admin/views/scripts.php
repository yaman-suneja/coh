    ...
    jQuery(document).ready(function($) {
        ...
        jQuery('#wpcgai_load_plugin_settings').on('click', function(e) {
            ...
            * */
            ...
            if(wpaicg_meta_custom_prompt_enable){
                ...
                wpaicg_generatorProcess(oldStep,'Generating Content');
            }
            else{
                oldStep = 'heading';
                ...
                if(response.status === 'success'){
                    if (wpai_modify_headings == 1 && is_generate_continue == 0 && !wpaicg_meta_custom_prompt_enable) {
                        ...
                        if(wpaicg_meta_custom_prompt_enable){
                            ...
                            wpaicg_generatorProcess(data.step, 'Generating SEO');
                        }
                        else{
                            data.step = 'content';
                            wpaicg_generatorProcess(data.step, 'Generating Content');
        ...
        function wpaicgStepWorking(data,timer){
            ...
            $.post(ajaxurl, data, function(response){
                ...
                wpaicg_generator_process.find('.wpaicg-generating-process-'+oldStep).addClass('wpaicg-error');
                wpaicg_generator_process.find('.wpaicg-generating-process-'+oldStep+' span').html('Error');
                wpaicg_ShowError('It appears that your web server has some kind of timeout limit. This can also occur if you are using a VPS, CDN, proxy, firewall, or Cloudflare. To resolve this issue, please contact your hosting provider and request an increase in the timeout limit. Additionally you can enable Custom Prompts for faster response to avoid timeout.', timer);
                $('#wpcgai_load_plugin_settings').removeAttr('disabled');
                $('#wpcgai_load_plugin_settings').find('.spinner').remove();
            });
        }
        ...
        $('.wpaicg_meta_custom_prompt').on('input', function (e){
            ...
            if(prompt.indexOf('[keywords_to_include]') > -1 || prompt.indexOf('[keywords_to_avoid]') > -1){
                $('.wpaicg_meta_custom_prompt_auto_error').html('<div style="color: #f00">Please note that keywords are only available in pro plan. Please remove keywords from your prompt</div>');
                $('#wpcgai_load_plugin_settings').attr('disabled','disabled');
            }
            else{
                $('.wpaicg_meta_custom_prompt_auto_error').empty();
